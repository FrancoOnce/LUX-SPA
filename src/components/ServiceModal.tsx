...
export function ServiceModal({ service, onClose }: ServiceModalProps) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={`Detalles de ${service.name}`} className="fixed inset-0 z-[70] flex items-end justify-center sm:items-center">
      ...
      <div
        role="presentation"
        className={`relative z-10 w-full max-w-lg overflow-hidden rounded-t-3xl border border-beige-300/35 bg-night-900/95 shadow-2xl backdrop-blur-2xl sm:rounded-3xl ${
      ...
      >
        ...
        <div className="p-6">
          ...
          <h3 className="mt-1 font-display text-2xl font-bold text-cream-100">{service.name}</h3>
          ...
          <div className="mt-6 flex items-center justify-between rounded-2xl border border-beige-300/35 bg-white/80 p-4">
            <div>
              <p className="text-xs uppercase tracking-wider text-beige-300">Precio desde</p>
              <p className="font-display text-2xl font-bold text-purple-600">S/ {service.price}</p>
            </div>
